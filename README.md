...
- 스킬 설치/인증 경로를 `openclaw skill install` + `openclaw config set skills.entries...` 기준으로 통일
...
| **스킬 시스템** | ✅ 완료 | 50개 내장 + OpenClaw CLI 기반 외부 설치 |
...
├─ AIAgent          Phase 2: AI API 호출 + 스킬 브라우저 + 인증 + 외부 스킬 설치 가이드
...
**Phase 2 (Step 5~) — 실제 AI 에이전트.** 모든 대화가 사용자가 선택한 AI의 실제 응답입니다. 에이전트는 JSON 형식으로 응답하며, 프론트엔드가 `actions` 배열을 파싱하여 스킬 체크, 크론 추가, 보안 설정, 토큰 요청, 외부 스킬 설치 가이드, 에이전트 추가 등을 실행합니다. 대화가 끝나면 인증이 필요한 스킬을 순회하며 토큰 입력 또는 "나중에" 선택을 받습니다.
...
58개 내장 스킬이 11개 카테고리로 분류되어 있습니다. 각 스킬에는 인증 방식 (없음/API Key/OAuth/PAT/CLI), 설명, 키 발급 URL 등의 메타데이터가 포함되어 있습니다.
...
내장 목록에 없는 스킬은 자동 검색하지 않습니다. 필요한 경우 `openclaw skill install <skill>` 또는 `openclaw skills install <skill>` 명령으로 외부 스킬을 직접 설치할 수 있습니다.
...
- **네트워크 호출은 한 곳에만:** AI 프로바이더 API (Phase 2 대화)
...
위험할 수 있습니다. 그래서 **`skill-scanner`가 자동 장착**됩니다. 외부 네트워크 호출, 리버스 셸, 프롬프트 인젝션, 권한 상승 등을 스킬 설치 전에 탐지합니다. 신뢰 가능한 저장소의 다운로드 수와 리뷰를 확인하는 것도 권장합니다.
...
- 새 스킬 → `Config.SKILLS`에 엔트리 추가 (내장 메타데이터 기준)
- **bat/sh 수정 시:**
